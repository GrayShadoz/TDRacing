...
def render(cam_x, cam_y, screen, player):
    ...
    for bullet in bullets:
        bullet.render(cam_x, cam_y, screen)
    player.render(cam_x, cam_y, screen)

# Update Function
def update(player):
    mouse_pos = pygame.mouse.get_pos()
    angle = get_angle_between((player.x+(player.rect.width/2)+cam_x,player.y+(player.rect.height/2)+cam_y),mouse_pos)

    # Create Bullet
    if pygame.mouse.get_pressed()[0]:
        tempBullet = Bullet(angle, player.x + (player.rect.width / 2), player.y + (player.rect.height / 2))
